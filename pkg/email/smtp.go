...
// s.SendMail("notify@a.com", "ulric@b.com;rain@c.com", "subject", "body, <font color=red>red</font>")
package email
...
func (s *SMTP) SendMail(from, tos, subject, body string, contentType ...string) error {
	...
	header := map[string]string{
		...
		"Subject":                   fmt.Sprintf("=?UTF-8?B?%s?=", contentEncoding.EncodeToString(typeconverter.UnsafeStrToBytes(subject))),
		"MIME-Version":              "1.0",
	...
	message += "\r\n" + contentEncoding.EncodeToString(typeconverter.UnsafeStrToBytes(body))
	return smtp.SendMail(
		address,
		smtp.PlainAuth("", s.Username, s.Password, server),
		from,
		tosArr,
		typeconverter.UnsafeStrToBytes(message),
	)
}
